...
Open the printed local URL, enter a repository path such as `D:\GitHub\log-pilot`, and click the analyze button. You can still run the CLI directly:
...
The local Web UI exposes `POST /api/scan` for path-based analysis and reads `report.json` plus `changes.diff` from the selected repository. The MVP intentionally uses Python standard library modules only so it can run on the current local Python 3.14 environment. Future integrations can add Typer, FastAPI, Pydantic, and Tree-sitter behind the existing CLI, Web, model, and parser boundaries.
